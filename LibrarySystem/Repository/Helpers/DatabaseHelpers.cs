...
{
    internal static class DatabaseHelpers<T> where T : IEntity
    {
        /// <summary>
        ...
        /// <param name="url">The url to use to get the database from.</param>
        /// <returns>Returns a MongoDatabase from the specified url.</returns>
        internal static SequenceRepository GetDatabaseSequenceFromUrl(MongoUrl url, string sequenceVal)
        {
            var client = new MongoClient(url);
            SequenceRepository _sequenceRepo = new SequenceRepository(client.GetDatabase(url.DatabaseName));
